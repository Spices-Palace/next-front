...
export const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getReportByDate: builder.query<any, string>({
      query: (date) => `/v1/reports/date/${date}`,
      providesTags: (result, error, date) => [{ type: 'Report', id: date }],
    }),
    generateReport: builder.mutation<any, { date: string }>({
      query: ({ date }) => ({
    ...
    getSalesmen: builder.query<any[], void>({
      query: () => '/v1/salesmen',
      providesTags: ['Salesman'],
    }),
    getBuyers: builder.query<any[], void>({
      query: () => '/v1/buyers',
      providesTags: ['Buyer'],
    }),
